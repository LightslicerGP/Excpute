...
def num2bin(num:int, bits:int=8):
    try:
...
def reg_read(id: str):
    with open("Registers.bin", "rb") as f:
        return f.read()[bin2num(id)]


def reg_write(id: str, data: str):
    try:
        with open("Registers.json", "r") as register_file:
            registers = json.load(register_file)
        registers[str(bin2num(id))] = data
...
instructions = [file_contents[i : i + 16] for i in range(0, len(file_contents), 16)]
print(instructions)

for adress in range(len(instructions)):
    flag = 0
    instruction = instructions[adress]
    instruction_opcode = instruction[:5]
    if instruction_opcode == "00000":
        # No operation
        print("hey, there is no operation lmao")
    elif instruction_opcode == "00001":
        # Halt
        print("Found instruction 00001, halting CPU.")
        break
    elif instruction_opcode == "00010":
        # Add
        ...
        result = bin(reg_a_data + reg_b_data)
        print(result)
        # print(f"Added register {int(reg_a, 2)} valued {reg_a_data} and register {int(reg_b, 2)} valued {reg_b_data} resulting {int(reg_a_data, 2) + int(reg_b_data, 2)} saving result in reg {int(reg_dest, 2)}")
        ...
        result = bin((int(reg_a_data, 2) - int(reg_b_data, 2)) & ((1 << max_bits) - 1))[
            2:
        ].zfill(max_bits)
        reg_write(reg_dest, result)
        print(
            f"Subtracted register {int(reg_a, 2)} valued {int(reg_a_data, 2)} and register {int(reg_b, 2)} valued {int(reg_b_data, 2)} resulting {int(reg_a_data, 2) - int(reg_b_data, 2)} saving result in reg {int(reg_dest, 2)}"
        )

    elif instruction_opcode == "00100":
        # Multiply
        reg_dest = instruction[5:8]
        flag_state = instruction[8:9]
        reg_a = instruction[10:13]
        ...
        reg_a_data = reg_read(reg_a)
        reg_b_data = reg_read(reg_b)
        result = bin(int(reg_a_data, 2) * int(reg_b_data, 2))[2:].zfill(reg_len)
        if len(result) == reg_len:
            reg_write(reg_dest, result)
            print(
                f"Multiplied register {int(reg_a, 2)} valued {int(reg_a_data, 2)} and register {int(reg_b, 2)} valued {int(reg_b_data, 2)} resulting {int(reg_a_data, 2) * int(reg_b_data, 2)} saving result in reg {int(reg_dest, 2)}"
            )
        else:
            if flag_state == "1":
                flag = result[:1]
                ...
                reg_write(reg_dest, result)
                print(
                    f"Multiplied register {int(reg_a, 2)} valued {int(reg_a_data, 2)} and register {int(reg_b, 2)} valued {int(reg_b_data, 2)} resulting {int(reg_a_data, 2) * int(reg_b_data, 2)} which is too large, saving {int(reg_dest, 2)}"
                )
            elif flag_state == "0":
                result = result[1:]
                reg_write(reg_dest, result)
    elif instruction_opcode == "00101":
        # Divide
        reg_dest = instruction[5:8]
        ...
        if squared_number_dec > 255:
            ...
            squared_number_bin = bin(squared_number_dec)[2:].zfill(8)

    elif instruction_opcode == "11010":
        # Load Immediate
        reg_dest = instruction[5:8]
